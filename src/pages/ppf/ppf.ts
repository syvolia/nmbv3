...
/**
 * Generated class for the PpfPage page.
 *
...
@Component({
  selector: 'page-ppf',
  templateUrl: 'ppf.html',
})
export class PpfPage {
  title:string="";
  accounts:any=[];
  objectblock:any={};
  body:any;
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad PpfPage');
  }
  makeRequest(){
      ...
      this.apiConnect.load(body,header).then(data => {
        ...
        if(this.data.error==""){
          ...
          if(response.field39=="00"){
            ...
            this.clientName=response.field48;
          }
          else{
            let template="<div>Transaction failed </div>";
            let obj={body:this.body, template:template, completed:true, pageTo:'MainPage'};
